...
function render() {
    ...
    var wordTiles = model.wordSubmissions.map(wordSubmissionChip)
    $("#word-submissions").append(wordTiles);
    ...
    if (disallowedLetters.length > 0) {
        ...
        $("#errorLetters").append(redLetterChips);
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        if (wordSubmission.isRealWord === true) {
            var wordValue = wordScore(wordSubmission.word);
            scoreChip = $("<span class = ' tag tag-sm tag-primary'></span>").text(" " + wordValue);
        } else {
...
function containsOnlyAllowedLetters(word) {
    ...
    if (letters.filter(isDisallowedLetter) > -1) {
        return true;
    } else {
        return false;
    }
...
function currentScore() {
    ...
    var realScore = wordScores.reduce(add, 0 );
    return realScore;
}
